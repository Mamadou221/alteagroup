...
export default function AgronomieProjectPage() {
  const { t } = useLanguage();

  // Structure flexible pour galerie (vide pour l'instant, à remplir plus tard)
  const galleryImages = t("projectPages.agronomy.gallery") || [];
  // Structure flexible pour chiffres clés (vide pour l'instant, à remplir plus tard)
  const stats = t("projectPages.agronomy.stats") || [];
  // Structure flexible pour études de cas (vide pour l'instant, à remplir plus tard)
  const caseStudies = t("projectPages.agronomy.caseStudies") || [];

  return (
    <main className="min-h-screen bg-white dark:bg-neutral-950">
      <ProjectHero
        projectId="agronomie"
        title={t("projectPages.agronomy.title")}
        subtitle={t("projectPages.agronomy.subtitle")}
      />

      <Section background="default">
        {/* Présentation du domaine */}
        <ProjectSection title={t("projectPages.agronomy.presentationTitle")} delay={0.1}>
          <p className="text-lg text-neutral-700 dark:text-neutral-300 leading-relaxed mb-6">
            {t("projectPages.agronomy.presentationText")}
          </p>
          <div className="grid md:grid-cols-3 gap-6 mt-8">
            {t("projectPages.agronomy.highlights").map((highlight, i) => (
              <motion.div
                key={i}
        ...
        <ProjectStats
          stats={stats}
          title={t("projectPages.agronomy.statsTitle")}
        />

        {/* Galerie (si disponible) */}
        <ProjectGallery
          images={galleryImages}
          title={t("projectPages.agronomy.galleryTitle")}
        />
          ...
          >
            <div className="max-w-7xl mx-auto px-6">
              <h2 className="text-2xl md:text-3xl font-bold mb-12 text-center text-neutral-900 dark:text-white">
                {t("projectPages.agronomy.caseStudiesTitle")}
              </h2>
        ...
        {/* État des projets */}
        <ProjectSection title={t("projectPages.agronomy.statusTitle")} delay={0.3}>
          <div className="bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-800 rounded-xl p-8 text-center">
            <div className="inline-flex items-center gap-3 mb-4">
              ...
              <h3 className="text-xl font-semibold text-neutral-900 dark:text-white">
                {t("projectPages.agronomy.statusText")}
              </h3>
            </div>
            <p className="text-neutral-700 dark:text-neutral-300 max-w-2xl mx-auto">
              {t("projectPages.agronomy.statusDescription")}
            </p>
          </div>
        </ProjectSection>

        {/* Méthodologie & engagement */}
        <ProjectSection title={t("projectPages.agronomy.methodologyTitle")} delay={0.4}>
          <div className="space-y-6">
            {t("projectPages.agronomy.methodology").map((item, i) => (
              <motion.div
                key={i}
        ...
        <ProjectSection delay={0.6}>
          <div className="text-center bg-neutral-50 dark:bg-neutral-900 rounded-xl p-12">
            <h2 className="text-2xl md:text-3xl font-bold mb-4 text-neutral-900 dark:text-white">
              {t("projectPages.agronomy.ctaTitle")}
            </h2>
            <p className="text-lg text-neutral-700 dark:text-neutral-300 mb-8 max-w-2xl mx-auto">
              {t("projectPages.agronomy.ctaText")}
            </p>
            <Button href="/contact" size="lg">
              {t("projectPages.agronomy.ctaButton")}
            </Button>
          </div>
        </ProjectSection>
      </Section>
